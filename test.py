...
class Trainer:
    def __init__(self,
        ...
        self.writer = SummaryWriter('logs/geo2')
        ...
        for epoch in range(1, self.epochs + 1):
            for step, batch_data in enumerate(self.train_loader):
                ...
                if global_step % self.save_step == 0:
                    ...
                    report_dict,avg_loss = self.test()
                    self.writer.add_scalar('Loss/test', avg_loss, global_step)
                    self.writer.add_scalar('Metrics/precision', report_dict['macro avg']['precision'],global_step)
                    self.writer.add_scalar('Metrics/recall', report_dict['macro avg']['recall'], global_step)
                    self.writer.add_scalar('Metrics/f1-score', report_dict['macro avg']['f1-score'],global_step)
